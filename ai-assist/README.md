...
- Во вкладке «Роль и поведение» Editor/Owner настраивает единственную рабочую роль ассистента, сохраняет неизменяемые черновики и применяет выбранную версию; Viewer может просматривать историю. Отдельного каталога ролей нет: вкладка сразу открывает редактор или первичную настройку.
- На странице «База знаний» Editor/Owner работает с единым каркасом записи: название, тип «Инфо»/«Товар»/«Услуга», Markdown-описание и URL источника. Цена, артикул и другие характеристики хранятся в том же Markdown-блоке; в интерфейсе есть только публикация и снятие с публикации.
...
- На странице «Источники сайта» Editor/Owner задаёт URL и глубину, получает вложенное дерево из sitemap и всех навигационных меню главной страницы, после чего выбирает любые узлы и их потомков. Crawler не ищет schema-разметку и не классифицирует страницу: он безопасно собирает сырой текст, а worker передаёт его в AITUNNEL для удаления мусора, классификации и сборки Markdown-записи. Результаты публикуются только после выбора пользователем.
- В редакторе роли Editor/Owner может переключать текст сохранённых версий, задать тестовый вопрос и получить потоковый ответ AITUNNEL для выбранной revision. Preview ищет только по опубликованным знаниям проекта/локали, показывает источники и scores, но не применяет роль или настройки ассистента.
...
Текущий рабочий source flow принимает URL публичного сайта, выбранные в дереве разделы и параметры обхода. Сайт сначала превращается в сырой текст, затем ИИ очищает и нормализует его в унифицированные Markdown-записи БЗ. Feed- и file-ingestion остаются следующими задачами. Рабочая БД, приватный API, VPN и SSH магазина не подключаются. Полный обход сайта на каждое сообщение не выполняется. Решение и ограничения: [ADR-003](decisions/003-public-sources-for-pilot.md), [ADR-005](decisions/005-ai-normalized-raw-site-ingestion.md), [guides/data-sources.md](guides/data-sources.md).
...
- Один проект имеет одну рабочую роль ассистента и любое число её черновиков/версий; одновременно создать вторую неархивную роль нельзя.
- Рабочие источники базы знаний MVP: публичный URL сайта и ручная Markdown-запись. Feed и file ingestion запланированы, но ещё не входят в рабочий поток. Прямой доступ к БД и приватному API запрещён в pilot.
