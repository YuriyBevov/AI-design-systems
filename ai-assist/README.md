...
- На странице «Источники сайта» Editor/Owner задаёт URL и глубину, получает вложенное дерево из sitemap и всех навигационных меню главной страницы, после чего выбирает любые узлы и их потомков. Crawler не ищет schema-разметку и не классифицирует страницу: он безопасно собирает видимый текст, а worker передаёт его в AITUNNEL для удаления мусора, классификации и полной Markdown-записи без сокращения относящихся к странице фактов. Страницы проходят ограниченно-параллельный pipeline: по умолчанию одновременно выполняются до двух fetch и пяти AI-нормализаций с сохранением robots crawl delay. Ошибочные страницы автоматически проходят до трёх page-level попыток; оставшиеся ошибки можно одной кнопкой запустить отдельной очередью либо повторить точечно. Большой run можно приостановить и продолжить; приостановленный run можно окончательно остановить, а после подтверждения worker — удалить вместе с постраничными результатами. Созданные записи БЗ при этом сохраняются. Prompt сохраняется внутри источника и run, но его редактор временно скрыт в интерфейсе. Результаты публикуются только после выбора пользователем.
...
Текущий рабочий source flow принимает URL публичного сайта, выбранные в дереве разделы и параметры обхода. Сайт сначала превращается в сохраняемый очищенный видимый текст без технических DOM-узлов и явного шаблонного boilerplate, затем ИИ выполняет смысловую очистку и нормализует его в унифицированные Markdown-записи БЗ. Prompt нормализации хранится внутри настроек, но его редактор временно скрыт. Feed- и file-ingestion остаются следующими задачами. Рабочая БД, приватный API, VPN и SSH магазина не подключаются. Полный обход сайта на каждое сообщение не выполняется. Решение и ограничения: [ADR-003](decisions/003-public-sources-for-pilot.md), [ADR-005](decisions/005-ai-normalized-raw-site-ingestion.md), [ADR-007](decisions/007-editable-crawl-normalization.md), [ADR-008](decisions/008-resilient-and-controllable-crawl-runs.md), [ADR-009](decisions/009-pre-ai-dom-boilerplate-filtering.md), [guides/data-sources.md](guides/data-sources.md).
