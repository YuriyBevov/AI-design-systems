...
- имеет полный доступ к панели и всем проектам;
- управляет пользователями, назначениями на проекты, provider credential и публикациями;
- создаёт, приостанавливает, возобновляет и архивирует проекты.

### Пользователь

- видит только назначенные ему проекты и доступные данные этих проектов;
- не видит технический раздел «Компоненты» и раздел управления пользователями;
- не меняет настройки, secrets, prompts и знания и не публикует изменения.

Внешний продуктовый контракт содержит только роли `Администратор` и `Пользователь`. Внутренние project membership levels не выводятся в UI.
...
- Администратор автоматически получает доступ ко всем проектам; при создании проекта можно назначить обычных пользователей.
