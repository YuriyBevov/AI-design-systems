...
- Список поддерживает выбор всех найденных или отдельных записей и массовые действия
  `publish|unpublish|delete`; динамические колонки ограничены 320 px, текст переносится, а
  переполнение таблицы остаётся внутри горизонтального scroll-контейнера.
- Prompt-driven массовая постобработка выполняется в worker-е по всем или выбранным записям,
  создаёт новую immutable version и не меняет активную публикацию без явного действия оператора.
- Запуски постобработки сохраняются в истории; активный запуск поддерживает паузу, продолжение и
  кооперативную остановку, а терминальный запуск можно удалить без удаления созданных версий БЗ.
...
- HTML-crawler извлекает видимый текст без schema.org/JSON-LD/microdata classification. До передачи
  модели он удаляет технические узлы, навигацию, внешние header/footer, cookie/modal/social/pagination
  и другие явно маркированные шаблонные блоки; ссылки собираются до очистки и остаются доступны
  discovery. Отдельный AI-normalizer удаляет оставшийся смысловой мусор, сохраняет все уникальные
...
- Операторский prompt AI-normalizer хранится в настройках источника и run snapshot, но временно
  скрыт в интерфейсе. API позволяет создать новый AI-only run по сохранённому сырому тексту без
  повторного fetch сайта; неизменяемая safety policy не редактируется.
- Страницы обрабатываются независимым bounded pipeline: отдельные лимиты ограничивают параллельные
  fetch и AI-нормализацию, а robots crawl delay остаётся общим для запросов к сайту.
- Активный run можно приостановить: новые fetch не начинаются, уже начатые операции завершаются;
  продолжение сохраняет достигнутый прогресс.
  Приостановленный run можно необратимо остановить; после подтверждения остановки worker его можно
  удалить каскадно с page-level результатами, не удаляя уже созданные документы БЗ. Для ошибочной
  или изменившейся страницы можно создать отдельный run с повторным fetch одного URL.
