...
- Соблюдать DRY для UI-кнопок: обычное действие использует базовый класс `.button`, а кнопка только с иконкой — `.icon-button`. Размер, цвет, фон и рамка меняются только абстрактными модификаторами базового компонента.
- Контекстный класс кнопки допустим только для поведения или размещения и не дублирует визуальные стили `.button`/`.icon-button`; если такой задачи нет, контекстный класс не добавляется.
- Иконки интерфейса выполняются контролируемым SVG через `.ui-icon`; Unicode-символы, emoji и текстовые знаки вместо иконок не используются. Новые варианты кнопок и иконок добавляются в раздел панели `Компоненты`.
- Все одиночные выпадающие списки интерфейса используют общий кастомный `BaseSelect` на `reka-ui`; нативный `<select>` и отдельные визуальные реализации на страницах не используются. Варианты списка добавляются в раздел `Компоненты`.
