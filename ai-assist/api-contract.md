...
- Пользовательские `statusMessage` и `message` ошибок возвращаются на русском языке. Технические
  `code`, имена полей и значения enum не переводятся, чтобы сохранять стабильность контракта.
- Необработанная серверная ошибка возвращает нейтральное русское сообщение и не раскрывает текст
