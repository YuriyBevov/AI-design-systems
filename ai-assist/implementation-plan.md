...
URL-crawl pipeline пересмотрен 2026-09-15 по ADR-005, ADR-007, ADR-008 и ADR-009: discovery строит рекурсивное дерево до заданной
глубины из sitemap и меню главной страницы; crawler сохраняет очищенный body text и ссылки, не
интерпретирует schema.org/JSON-LD/microdata и до ИИ удаляет технические узлы и явно маркированный
навигационный/шаблонный boilerplate. Отдельный
AITUNNEL-вызов сохраняет все релевантные факты, очищает видимый шум, классифицирует страницу как
`info|product|service` и возвращает title/Markdown по строгой JSON-схеме. Сырой видимый текст и
снимок prompt-а сохраняются для AI-only reprocessing без повторного fetch; редактор prompt-а
временно скрыт. Run поддерживает паузу между страницами, окончательную остановку из паузы и удаление
остановленного run с page-level результатами без удаления созданных документов БЗ; строка
результата поддерживает отдельный повторный fetch и обработку одного URL. Результаты
по-прежнему подтверждаются вручную и индексируются атомарно. UI записей скрывает
