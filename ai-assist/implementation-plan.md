...
URL-crawl pipeline пересмотрен 2026-09-15 по ADR-005 и ADR-007: discovery строит рекурсивное дерево до заданной
глубины из sitemap и меню главной страницы; crawler сохраняет только сырой body text и ссылки, не
интерпретирует schema.org/JSON-LD/microdata и удаляет только невидимый технический код. Отдельный
...
снимок редактируемого prompt-а сохраняются для AI-only reprocessing без повторного fetch. Результаты
