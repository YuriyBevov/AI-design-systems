...
`id`, `display_name`, `email_normalized`, `password_hash`, `role(admin|user)`, `status`, `last_login_at`, timestamps.

`admin` — продуктовая роль с полным доступом ко всем проектам и техническим разделам. `user` получает доступ только к явно назначенным проектам и не видит технический раздел «Компоненты». Последнего активного администратора нельзя отключить или понизить; отключение любого пользователя отзывает его активные сессии.
...
`active` разрешает административные операции, public widget и фоновые задания. `suspended` сохраняет данные проекта, но немедленно исключает его из runtime, обычного project scope и новых worker-заданий; администратор может вернуть проект в `active`. Пользовательское удаление переводит проект в терминальное состояние `archived`: он исчезает из списка управления, но история, audit и tenant-owned данные сохраняются до отдельной retention/purge-процедуры.

Создание проекта всегда создаёт нового assistant с новым непредсказуемым `public_id` и собственным черновиком конфигурации. Все активные администраторы получают доступ автоматически, а выбранные в форме обычные пользователи — доступ на чтение. При выборе проекта-шаблона копируется только allowlist повторно используемых настроек: визуальная/поведенческая конфигурация assistant, его политика срока хранения диалогов и последние revision неархивированных prompts. Origins/домены, contact fallback, provider credentials, model settings, knowledge, publications, conversations, memberships и audit не копируются. Ключ провайдера остаётся project-scoped записью; один и тот же внешний ключ можно вручную сохранить в нескольких проектах, но у каждого будут собственные envelope, lifecycle и audit.

### `project_memberships`

`project_id`, `user_id`, internal `role(owner|editor|viewer)`, timestamps. Unique `(project_id, user_id)`. В текущей продуктовой модели `admin` отображается во все проекты как `owner`, а `user` — в выбранные проекты как `viewer`. `editor` сохранён только для совместимости существующей схемы и новым API не назначается.
