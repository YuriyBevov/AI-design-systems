...
Актуальное решение описано в [ADR-005](../decisions/005-ai-normalized-raw-site-ingestion.md) и
[ADR-007](../decisions/007-editable-crawl-normalization.md).
...
ссылок. Перед чтением текста удаляются только невидимые технические узлы `script`, `style`,
`noscript`, `template` и `svg`: их код не должен вытеснять описание страницы из ограниченного входа
модели. Crawler намеренно не:
...
JSON-формат. Отдельный операторский prompt виден в интерфейсе: его можно изменить при создании
источника, а после завершения run — отредактировать и запустить новый AI-only run по сохранённому
сырому тексту без повторного fetch сайта.
...
- raw extractor сохраняет видимый boilerplate, не читает schema markup и удаляет технический код,
  не обрезая идущее после него описание;
