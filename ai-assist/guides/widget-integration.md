...
Готовый snippet для активного проекта отображается первым блоком во вкладке «Подключение». Кнопка
копирования переносит весь код loader и custom element в буфер обмена; URL loader формируется из
публичного origin панели, а `assistant-id` — из выбранного проекта. Перед вставкой нужно добавить
точный origin клиентского сайта во вкладке «Безопасность» и применить настройки.
