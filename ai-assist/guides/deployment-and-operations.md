...
умолчанию — 300 секунд, допустимый диапазон — от 60 до 900 секунд.
`KNOWLEDGE_CRAWL_AI_IDLE_TIMEOUT_MS` прерывает поток, если от provider нет данных 60 секунд;
допустимый диапазон — от 10 до 120 секунд. Общий
...
`KNOWLEDGE_CRAWL_FETCH_CONCURRENCY` задаёт число одновременных fetch внутри run (по умолчанию `2`,
диапазон `1..10`). `KNOWLEDGE_CRAWL_AI_CONCURRENCY` ограничивает число одновременных crawl
AI-вызовов на один worker process (по умолчанию `5`, диапазон `1..20`). Увеличение требует сверки с
rate limit и бюджетом provider; crawl delay сайта при этом не отключается.

Долгие system jobs используют десятиминутный BullMQ lock, обновляют его раз в минуту и проверяют
stalled jobs раз в минуту. До окончательного перевода в `failed` допускаются три восстановления из
stalled-состояния. Это оставляет запас для временной задержки event loop или Redis и предотвращает
параллельный повтор ещё работающего многочасового crawl; после реального падения worker возврат job
в очередь может занять до одиннадцати минут. Запуски остаются идемпотентными за счёт page checkpoints
в PostgreSQL.
