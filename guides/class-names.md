...
Секция или группа может добавлять свой элемент для конкретной раскладки. Такой элемент отвечает за количество колонок, расстояния и правила размещения, но не за внутренние стили карточек.
...
```html
footer footer__inner footer__nav footer__contacts footer__socials
